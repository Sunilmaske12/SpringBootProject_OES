...
public class OrderController {
	...
	public String myMyOrders(Model model, @AuthenticationPrincipal CustomUserDetails userDetail) {
		...
		model.addAttribute("orderStatus", (String) httpsession.getAttribute("orderPlaceStatus"));
		httpsession.removeAttribute("orderPlaceStatus");
			return "MyOrders";
	}
	...
	public String getAdminOrderDetailsPage(Model model, @PathVariable("orderId") int oid,
			@PathVariable("addressId") int aid) {
	...
	public String checkoutPage(Model model, @AuthenticationPrincipal CustomUserDetails user) {
		...
		if (cartLength != null)
			model.addAttribute("cartLength", (Integer.parseInt(cartLength)));
		if (cartLength != null)
			model.addAttribute("totalAmount", (Integer.parseInt(totalAmount)));
	...
	public String makeOrder(@PathVariable("amount") int amount, @PathVariable("cartLength") int cartLength,
			Model model) {
		...
		return "redirect:/User/orderPlacePage,"+cartLength+","+address_id;
	}
	
	@GetMapping("/User/orderPlacePage,{cartLength},{address_id}")
	public String getPlaceOrderPage(Model model, @PathVariable("address_id") String address_id, @PathVariable("cartLength") int cartLength ) {
	...
	public ModelAndView settingOrderDetails1(@RequestParam(value = "pid[]") Integer[] pid,
			@RequestParam(value = "pname[]") String[] pname, @RequestParam(value = "pquantity[]") Integer[] pquantity,
			@RequestParam(value = "pprice[]") Integer[] pprice, HttpSession session) {
	...
	public String orderPlace(@AuthenticationPrincipal CustomUserDetails user) {
		...
		Integer[] pid = (Integer[]) httpsession.getAttribute("pid");
		Integer[] pprice = (Integer[]) httpsession.getAttribute("pprice");
		Integer[] pquantity = (Integer[]) httpsession.getAttribute("pquantity");
		String[] pname = (String[]) httpsession.getAttribute("pname");
		...
		httpsession.setAttribute("orderPlaceStatus", "Done");
